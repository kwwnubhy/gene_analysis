...
# 필요한 라이브러리 목록 넣어주세요. 
```{r}
need_libraries <- c('GEOquery','dplyr','tidyr','ggplot2', 'reshape2', 'dendextend', 'cluster', 'ComplexHeatmap')
```
...
for (lib in need_libraries) {
  if (!requireNamespace(lib, quietly = TRUE)) {  # 라이브러리가 설치되어 있는지 확인
    if (grepl("is not available", tryCatch(install.packages(lib), error = function(e) conditionMessage(e)))) {
      not_installed <- c(not_installed, lib)  # 설치되지 않은 경우 목록에 추가
    } else {
      if (grepl("package ‘(.+)’ is not available", conditionMessage(e), perl = TRUE)) {
        not_installed <- c(not_installed, sub("package ‘(.+)’ is not available", "\\1", conditionMessage(e)))
      }
    }
  }
}
...
if (length(not_installed) > 0) {
  for (lib in not_installed) {
    if (grepl("is not available", tryCatch(BiocManager::install(lib), error = function(e) conditionMessage(e)))) {
      cat(paste("Unable to install", lib, "\n"))
    }
  }
}
...
```{r}
# 파일이 있는 디렉토리 경로 지정
directory_path <- "/Users/iminju/Desktop/GAS6_confirm/RNA_seq/raw/GSE135705_unzip"
...
for (i in seq(1,length(DP_data))){
  ...
  DP_data[[i]] <- subset(DP_data[[i]], select = -Name)
...
# HFSC에서도 Geneid를 행 이름으로 이용하고자 하였으나, 중복된 Geneid가 존재하는 것을 확인. 따라서 전처리 후 이용.
```{r}
...
color_palette <- scale_color_continuous_sequential(palette='BluGrn')
