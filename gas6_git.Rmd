...
```{r}
# 파일이 있는 디렉토리 경로 지정
directory_path <- "C:/Users/kwwnu/Desktop/GAS6_confirm/RNA_seq/raw/GSE135705_unzip"
...
##    하기 전에 손으로 먼저 분석해보려구요...
```{r}
get_DEG <- function(using_data, control, test){
  control_df <- using_data[,control]
  control_df$mean <- mean(control_df[,1], control_df[,2])
  test_df <- using_data[,test]
  test_df$mean <- mean(test_df[,1], test_df[,2])
  fold_df <- test_df$mean / control_df$mean
}
```
